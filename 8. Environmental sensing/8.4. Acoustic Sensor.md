Acoustic Sensor 관련 교육자료를 만드는데 정리좀 해줄래 ? 문서 형식은 md 파일이고 큰 주제는 내가 정해뒀어. 여기에 내용을 알맞게 채워주면 되는데 고등학생이 공부할 수준이면 좋겠어. 물론 대학생도 가능하면 더 좋고. Acoustic은 Pi Zero 2w 에 ics43434 연결해서 주변 소음 측정해서 dBA 값을 전달하는 센서야. 센서가 Pizero 2w 가 있다는 설명등은 숨기고, 주변 소음관련된 설명을 자세하게 할려고해. 그리고 저기 있는 예제를 발전 시켜서 활용할 수 있는 예제가 2개정도 있으면 좋을꺼 같아. 이 내용들을 포함해서 마크다운 형태로 정리해줘. 마크다운에 설명되는 내용들은 문어체 기반에 존대를 포함해서 작성해줘.
